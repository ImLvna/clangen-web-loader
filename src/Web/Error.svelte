<script lang="ts">
  ...
  const blacklist = ["video system not initialized"];
    ...
    >
      ...
      <div class="flex flex-row gap-3">
        <button
          ...
          class="bg-gray-600 p-2 rounded-lg">Reload</button
        >
        <button onclick={window.toggleDebug} class="bg-gray-600 p-2 rounded-lg"
          >Open Logs</button
        >
        <button
          onclick={window.downloadLogsCallback}
          class="bg-gray-600 p-2 rounded-lg">Download Logs</button
        >
      </div>
    </div>
  </div>
